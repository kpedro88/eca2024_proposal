...
\begin{enumerate}
\item building new models to obtain a broader set of complete hidden valley theories (Section~\ref{subsec:models});
\item exploiting AI-based anomaly detection triggers to record more dark QCD events with less production mode and kinematic dependence (Section~\ref{subsec:trig});
\item further developing existing unsupervised AI techniques to tag SVJs more efficiently (Section~\ref{subsec:tagging};
\item extracting the signal using a new jet-based mass variable to reduce dependence on the production mode (Section~\ref{subsec:strategy}).
\end{enumerate}
...
\begin{figure}[htb!]
\centering
\twofigeqh{figures/efficiency_BESTandAD_schan_CICADA_score_v2p1p1.pdf}{figures/efficiency_BESTandAD_tchan0_CICADA_score_v2p1p1.pdf}
\caption{Improvements of up to 25--30\% in trigger efficiency for $s$-channel (left) and $t$-channel (right) SVJ production
from a preliminary version of the CICADA anomaly trigger with various rates.
The overall low trigger efficiency at low mediator mass is balanced by higher production cross sections.
...
Frequent monitoring of the anomaly triggers' output will be essential to ensure they are not misled by unusual instrumental noise or other unwanted effects.

\textit{Risk mitigation}: if the final anomaly trigger performance is not sufficient, the multiple trigger strategy can be used, at the cost of more dependence on event kinematics and more effort to analyze multiple datasets.
...
\begin{figure}[htb!]
\centering
\twofigeqh{figures/EXO-19-020_Figure_003-a.pdf}{figures/EXO-19-020_Figure_003-a.pdf}
