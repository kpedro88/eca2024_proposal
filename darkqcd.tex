...
In addition, because low-energy QCD bound state formation via showering and hadronization is non-perturbative, it must be simulated using phenomenological models~\cite{Carloni:2010tw},
...
Subsequently, the PI has developed several more searches using the LHC Run 2 dataset to target different mediator mass regimes:
...
a supervised (model-dependent) tagger using particle-level inputs, based on the ParticleNet graph neural network architecture~\cite{};
...
The PI was among the first to demonstrate the autoencoders could discriminate between SM QCD jets and SVJs, even surpassing a supervised tagger when applied to a different model~\cite{}.
To achieve discrimination against \ttbar jets, the normalized autoencoder (NAE) formalism~\cite{} was adopted in order to minimize complexity bias.
...
a graph-based autoencoder (GAE), using the Lund plane representation following Refs.~\cite{} and trained with the WNAE approach.
...
the semi-supervised universal domain adaptation technique developed by the PI and his collaborators~\cite{}
will be employed to ensure similar performance on data by only allowing physically invariant features to be learned.
...
The CMS Dark QCD team has recently released new results for both signatures in Refs.~\cite{},
...
The best strategies for intermediate or large values of \thooft in combination with the other parameters remain an open question.
The generation of physically realistic events at intermediate \thooft has only recently become possible~\cite{Cesarotti:2020uod}.
...
Much like the pMSSM, this scan will include many random parameter combinations, with existing constraints included to avoid models that are already ruled out.
...
\end{enumerate}
Ultimately, these elements provide additional gains over the use of the Run 3 dataset, which is larger and higher energy than the Run 2 dataset.
The basic discovery potential grows by up to X\% when going from $\sqrt{s} = 13$ to 13.6\TeV and integrated luminosity of 138 to ${\sim}$190--260\fbinv.
