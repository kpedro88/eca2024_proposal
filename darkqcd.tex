...
resulting in a generalized function that applies to model parameter variations and even other processes not included in training, rather than a simple regression.
Figure~\ref{fig:svjmass} (left) shows an example result for \Pbifun pair production, including the performance on SM QCD multijet events;
other studies show that the EVN is robust against signal model parameter variations.
While this technique improves sensitivity compared to existing strategies, different mass variables are still needed for different production modes, with \ptmiss used in the nonresonant case.

We propose to go further by unifying the offline analysis strategy to cover all production modes.
We will extend this concept to a jet variable network (JVN) by incorporating a graph neural network (GNN) to analyze the jet constituents.
...
To estimate the SM background, we will employ the new ``closure loss'' developed by the PI and his colleagues
...
Alternatively, we will fall back to the event-level strategy, still using the optimal mass variables learned by the EVN, at the cost of additional effort and dependence on production mode.
Even then, the closure-loss background estimation technique will be employed for all production modes.
